...
CHAR_P File::Dirname(CCHAR_P szPath)
{
	...
	{
		CHAR_P szDirname = strdup(szPath);
		// Firx for valgrind `Invalid read of size 4` on Linux
		CHAR_P szEnd = szDirname;
		while (*szEnd++ != '\0') { ;; }
		--szEnd;

		while (szEnd != szDirname && *szEnd != '/') { *szEnd-- = '\0'; }
		if (szEnd == szDirname) { *szDirname = '.'; }
