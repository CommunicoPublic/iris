                         ...
                         const bool    bWithLineFeed)
{
	...
	for (;;)
	{
		if (iPos != pSearchBuffer -> data_size)
		{
			...
			if (bEOLFound)
			{
				if (bWithLineFeed) { oLine.Append(pSearchBuffer -> data + iStartPos, iPos - iStartPos + 1); }
				else               { oLine.Append(pSearchBuffer -> data + iStartPos, iPos - iStartPos);     }
				break;
			}
			else
			{
