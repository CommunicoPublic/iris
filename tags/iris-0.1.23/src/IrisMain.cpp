...
static ConfigHelper::State ParseIncludes(const STLW::vector<STLW::string>  & vIncludeDirs,
                                         ...
                                         const STLW::vector<STLW::string>  & vConfigs,
                                         ...
                                         Logger                            & oLogger)
{
	...
	{
		ProcPipe oProcPipe;
		if (oProcPipe.Open(itvConfigs -> c_str()) == -1) { return ConfigHelper::ERROR; }

		STLW::string sData;
		CHAR_8 szBuf[4096];
		for (;;)
		{
			INT_64 iBytes = oProcPipe.Read(szBuf, 4096);
			if (iBytes == 0) { break; }
			if (iBytes == -1) { return ConfigHelper::ERROR; }

			sData.append(szBuf, iBytes);
		}

		STLW::vector<STLW::string> vIncludePatterns2;
		STLW::vector<STLW::string> vConfigs2;
		ConfigHelper::State oConfigState = ConfigHelper::ParseConfigData(*itvConfigs, sData, vIncludePatterns2, vConfigs2, sSections, oConfigData, oLogger);
		if (oConfigState != ConfigHelper::OK) { return oConfigState; }

		if (!vIncludePatterns2.empty() || !vConfigs2.empty())
		{
			oConfigState = ParseIncludes(vIncludeDirs, vIncludePatterns2, vConfigs2, sSections, oConfigData, sFiles, oLogger);
			if (oConfigState != ConfigHelper::OK) { return oConfigState; }
		}
		...
		while (itvIncludePatterns != vIncludePatterns.end())
		{
			...
			while (itvFiles != vFiles.end())
			{
				...
				else
				{
					...
					STLW::vector<STLW::string> vIncludePatterns2;
					STLW::vector<STLW::string> vConfigs2;
					ConfigHelper::State oConfigState = ConfigHelper::ParseConfigFile(sRealPath, vIncludePatterns2, vConfigs2, sSections, oConfigData, oLogger);
					if (oConfigState != ConfigHelper::OK) { return oConfigState; }
...
int main(int argc, char ** argv)
{
	...
	STLW::set<STLW::string>    sSections;
	ConfigHelper::State oConfigState = ConfigHelper::ParseConfigFile(sConfigFile, vIncludePatterns, vConfigs, sSections, oConfigData, oLogger);
	...
	oConfigState = ParseIncludes(vIncludeDirs, vIncludePatterns, vConfigs, sSections, oConfigData, sFiles, oLogger);
	if (oConfigState != ConfigHelper::OK)
	{
		if (oConfigState == ConfigHelper::ERROR)        { return EX_SOFTWARE; }
		if (oConfigState == ConfigHelper::CONFIG_ERROR) { return EX_CONFIG; }
