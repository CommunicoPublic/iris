...
static void * Producer(void * pContext)
{
	...
	fprintf(stderr, "Starting Producer: %llu\n", (long long unsigned)pthread_self());
	...
	for (;;)
	{
		...
		fprintf(stderr, "Producer: %llu -> %d\n", (long long unsigned)pthread_self(), pThreadContext -> data);
		++pThreadContext -> data;
	...
	fprintf(stderr, "Ending Producer: %llu\n", (long long unsigned)pthread_self());
return pContext;
}
...
{
	ThreadContext * pThreadContext = (ThreadContext *)pContext;
	fprintf(stderr, "Starting Consumer: %llu\n", (long long unsigned)pthread_self());
	...
	fprintf(stderr, "Ending Consumer: %llu\n", (long long unsigned)pthread_self());
return pContext;
}
