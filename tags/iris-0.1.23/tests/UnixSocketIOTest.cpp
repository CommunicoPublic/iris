...
{
	ClientTCPSocket * pClientSocket = new ClientTCPUnixSocket("/tmp/socket-test.sock", 0644);
	...
	dynamic_cast<ClientTCPUnixSocket *>(pClientSocket) -> Connect();

	// Poller for client socket
	SocketPoller<ClientTCPSocket *> oSocketPoller(2);
...
int main(int argc, char ** argv)
{
	...
	ServerTCPSocket * pServerSocket = new ServerTCPUnixSocket("/tmp/socket-test.sock", 0644);
