...
// Constructor
//
ServerTCP6SSLSocket::ServerTCP6SSLSocket(const bool       bIStartTLS,
                                         CCHAR_P          szIHost,
                                         const UINT_32    iIPort): ServerTCPSSLSocket(bIStartTLS),
                                                                   szHost(NULL),
                                                                   szIP(NULL),
                                                                   iPort(iIPort)
{
...
// Open socket
//
TCPSocket::State ServerTCP6SSLSocket::Open()
{
...
// Close socket
//
TCPSocket::State ServerTCP6SSLSocket::Close()
{
	close(iSocket);
	iSocket = -1;
...
// Bind to the address
//
TCPSocket::State ServerTCP6SSLSocket::Bind()
{
...
//
TCPSocket::State ServerTCP6SSLSocket::Accept(ClientTCPSocket *  & pServerClientSocket)
{
	...
	pServerClientSocket = new ServerClientTCP6SSLSocket(!bStartTLS, iClientSocket, sBuffer, ntohs(sSocketAddress.sin6_port));
...
// Get address family
//
TCPSocket::AddressFamily ServerTCP6SSLSocket::GetAddressFamily() const { return INET6; }

//
// Destructor
//
ServerTCP6SSLSocket::~ServerTCP6SSLSocket() throw()
{
	Close();
	free(szIP);
	free(szHost);
